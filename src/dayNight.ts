...
export const DAY_NIGHT_CYCLE_SEC = 120

export const BRIGHTNESS_MIN = 0.42
...
export function sunAnglesFromPhase(phase: number): { elevationDeg: number; azimuthDeg: number } {
  ...
  const elevationDeg = dayArc * 58 - 8
...
export function skyAtmosphereFromPhase(phase: number, cycleT: number): SkyAtmosphere {
  ...
  const day = smooth01((cycleT - 0.42) / 0.28)
  ...
  out.turbidity = THREE.MathUtils.lerp(out.turbidity, 1.1, night * 0.55)
  out.rayleigh = THREE.MathUtils.lerp(
    0.35,
    THREE.MathUtils.lerp(2.5, THREE.MathUtils.lerp(2.8, 3.4, golden), golden),
    day,
  )
  out.rayleigh = THREE.MathUtils.lerp(out.rayleigh, 0.28, night * 0.4)
  ...
  lerpColor(out.sunColor, 0x6a7a9a, 0xfff4e8, day)
  out.sunColor.lerp(_cA.setHex(goldenSun), golden * 0.92)
  out.sunColor.lerp(_cA.setHex(0x8a9ec0), night * 0.72)
  ...
  lerpColor(out.hemisphereSky, 0x0c1228, 0xc8dff8, day)
  ...
  out.hemisphereSky.lerp(_cA.setHex(0x1a2848), night * 0.88)
  out.hemisphereSky.lerp(_cA.setHex(0x283060), night * 0.35)

  lerpColor(out.hemisphereGround, 0x12101c, 0x6a5848, day)
  out.hemisphereGround.lerp(_cA.setHex(0x4a4848), golden * 0.28)
  out.hemisphereGround.lerp(_cA.setHex(0x0a0812), night * 0.72)

  lerpColor(out.ambientColor, 0x3a4868, 0xd0dce8, day)
  ...
  out.ambientColor.lerp(_cA.setHex(0x98a8d0), night * 0.38)

  lerpColor(out.fillColor, 0x283850, 0xb8d8ff, day)
  ...
  out.fillColor.lerp(_cA.setHex(0x506888), night * 0.38)

  lerpColor(out.fill2Color, 0x241c28, 0xfff0e0, day)
  ...
  out.fill2Color.lerp(_cA.setHex(0x384058), night * 0.3)

  lerpColor(out.fogColor, 0x182038, 0x8ab4d8, day)
  ...
  out.fogColor.lerp(_cA.setHex(0x2a3858), night * 0.62)
